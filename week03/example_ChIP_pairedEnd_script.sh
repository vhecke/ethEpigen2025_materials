...
(bowtie2 -p $ncpus --dovetail --no-mixed --no-discordant -I 15 -X 2000 -x $ref \ 
  ...
  samtools view -bS - | samtools sort -@ $ncpus -m 2G - > $bam
...
if [ -f "tracks/$base.bw" ]; then
  ...
  bamCoverage -p $ncpus --ignoreDuplicates --effectiveGenomeSize 2652783500 --normalizeUsing CPM -b aligned/"$base".bam -o tracks/$base.bw
fi
